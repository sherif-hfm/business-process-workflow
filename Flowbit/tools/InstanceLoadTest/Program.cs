...
Console.WriteLine("Flowbit instance load test");
...
{
    Console.WriteLine("""
        Usage: dotnet run --project Flowbit/tools/InstanceLoadTest -- [options]
        ...
        Example:
          ...
          dotnet run --no-launch-profile --project Flowbit/src/Flowbit.Api -- --environment LoadTest --urls http://localhost:5017

          # Warm up, then run the measured workload:
          dotnet run --project Flowbit/tools/InstanceLoadTest -- --count 1000 --concurrency 48
          dotnet run --project Flowbit/tools/InstanceLoadTest -- --count 200000 --concurrency 48
        """);
}
