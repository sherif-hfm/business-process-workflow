...
sealed class ManagedApiHost : IAsyncDisposable
{
    ...
    public ManagedApiHost(Options options)
    {
        ...
        _projectPath = Path.GetFullPath(options.ApiProject
            ?? Path.Combine(options.FixtureRoot!, "Flowbit", "src", "Flowbit.Api", "Flowbit.Api.csproj"));
    ...
    public async Task StartAsync()
    {
        ...
        if (!_process.Start()) throw new InvalidOperationException("Failed to start Flowbit.Api.");
    ...
    private async Task WaitUntilReadyAsync()
    {
        ...
        while (DateTimeOffset.UtcNow < deadline)
        {
            if (_process is { HasExited: true })
                throw new InvalidOperationException($"Flowbit.Api exited during startup with code {_process.ExitCode}. See {_logPath}.");
            try
            {
        ...
        throw new TimeoutException($"Flowbit.Api did not become ready. Last error: {lastError?.Message}. See {_logPath}.");
    }
